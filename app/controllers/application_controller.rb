class ApplicationController < ActionController::Base
  ...
  def current_pool
    ...
    @current_pool = current_player.pools.find_by(id: session[:pool_id]) || general_pool_for(Tournament.active)
  end

  # La polla General pública de un torneo, creándola si hace falta.
  def general_pool_for(tournament)
    return nil unless tournament
    Pool.general_for(tournament) ||
      Pool.create!(name: "#{tournament.name} — General", tournament: tournament, public: true)
  end

  def ensure_general_membership
    pool = general_pool_for(Tournament.active)
