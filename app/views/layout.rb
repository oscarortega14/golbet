      ...
      header(class: "sticky top-0 z-30 border-b border-border bg-background/70 backdrop-blur") do
        div(class: "mx-auto flex max-w-3xl items-center justify-between gap-3 px-4 py-3") do
          ...
          div(class: "flex items-center gap-2") do
            ...
            a(href: tournaments_path, class: "text-sm text-muted-foreground hover:text-foreground transition") { "Torneos" }
            a(href: pools_path, class: "text-sm text-muted-foreground hover:text-foreground transition") { "🏆 #{helpers.current_pool&.name || "Pollas"}" }
